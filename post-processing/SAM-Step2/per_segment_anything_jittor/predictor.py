...
class SamPredictor:
    ...
    def set_image(
        ...
        input_image_torch = jt.array(input_image).stop_grad()
        ...
        if mask is not None:
          ...
          input_mask_torch = jt.array(input_mask).stop_grad()
    ...
    def set_torch_image(
        ...
        original_image_size: Tuple[int, ...],
        transformed_mask: jt.array = None,
        cal_image=True
    ) -> None:
        """
        Calculates the image embeddings for the provided image, allowing
    ...
    def predict(
        ...
        if point_coords is not None:
            ...
            coords_torch = jt.array(point_coords).float32().stop_grad()
            labels_torch = jt.array(point_labels).float32().stop_grad()
            coords_torch, labels_torch = coords_torch[None, :, :], labels_torch[None, :]
        if box is not None:
            box = self.transform.apply_boxes(box, self.original_size)
            # box_torch = torch.as_tensor(box, dtype=torch.float, device=self.device)
            box_torch = jt.array(box).float32().stop_grad()
            box_torch = box_torch[None, :]
        if mask_input is not None:
            # mask_input_torch = torch.as_tensor(mask_input, dtype=torch.float, device=self.device)
            # mask_input_torch = jt.array(mask_input).float32()
