...
from src.decoder_head.wrappers import resize
...
def main_worker(args):
    ...
    # build dataset
    assert args.mode in ['train', 'validation', 'test']
    ...
    for images, path, height, width in tqdm(dataloader):
        ...
        with jt.no_grad():
            ...
            prediction = aug_test(model, images, H, W)
...
def aug_test(model, images, H, W, flip_direction='horizontal'):
    output = model(images)
    if flip_direction == 'horizontal':
        output_flip = model(jt.flip(images, dim=(3, )))
        output += jt.flip(output_flip, dim=(3, ))
    elif flip_direction == 'vertical':
        output_flip = model(jt.flip(images, dim=(2, )))
        output += jt.flip(output_flip, dim=(2, ))
    output /= 2

    images_scale1 = resize(images, (384, 384), mode='bilinear', align_corners=False)
    output_scale1 = model(images_scale1)
    if flip_direction == 'horizontal':
        output_scale_flip1 = model(jt.flip(images_scale1, dim=(3, )))
        output_scale1 += jt.flip(output_scale_flip1, dim=(3, ))
    elif flip_direction == 'vertical':
        output_scale_flip1 = model(jt.flip(images_scale1, dim=(2, )))
        output_scale1 += jt.flip(output_scale_flip1, dim=(2, ))
    output_scale1 /= 2

    images_scale = resize(images, (512, 512), mode='bilinear', align_corners=False)
    output_scale = model(images_scale)
    if flip_direction == 'horizontal':
        output_scale_flip = model(jt.flip(images_scale, dim=(3, )))
        output_scale += jt.flip(output_scale_flip, dim=(3, ))
    elif flip_direction == 'vertical':
        output_scale_flip = model(jt.flip(images_scale, dim=(2, )))
        output_scale += jt.flip(output_scale_flip, dim=(2, ))
    output_scale /= 2

    if H * W > args.max_res * args.max_res and args.max_res > 0:
        output = nn.interpolate(output, (args.max_res, int(args.max_res * W / H)), mode="bilinear", align_corners=False)
        output_scale = nn.interpolate(output_scale, (args.max_res, int(args.max_res * W / H)), mode="bilinear", align_corners=False)
        output_scale1 = nn.interpolate(output_scale1, (args.max_res, int(args.max_res * W / H)), mode="bilinear", align_corners=False)
        output = output + (output_scale + output_scale1) / 2
        # output = output + output_scale
        output = jt.argmax(output/2, dim=1, keepdims=True)[0]
        prediction = nn.interpolate(output.float(), (H, W), mode="nearest").long()
    else:
        output = nn.interpolate(output, (H, W), mode="bilinear", align_corners=False)
        output_scale = nn.interpolate(output_scale, (H, W), mode="bilinear", align_corners=False)
        output_scale1 = nn.interpolate(output_scale1, (H, W), mode="bilinear", align_corners=False)
        output = output + (output_scale + output_scale1) / 2
        # output = output + output_scale
        prediction = jt.argmax(output/2, dim=1, keepdims=True)[0]
