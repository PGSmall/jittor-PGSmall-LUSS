...
class EvalDataset(datasets.ImageFolder):
    ...
    def __getitem__(self, item):
        ...
        return target, boundary_target, prediction, boundary_prediction, logit, self.prediction_lst[item]
...
class InferImageSamFolder(datasets.ImageFolder):
    """Dataset for the inference stage."""
    def __init__(self, root, transform, num_gpus=1):
        super().__init__(root, transform=transform)
...
class PseudoLabelSamDataset(datasets.ImageFolder):
    """Dataset for the finetuing stage."""
    def __init__(
        self,
        root,
        ...
        super(PseudoLabelSamDataset, self).__init__(root, transform)
        self.pseudo_path = pseudo_path
    ...
    def __getitem__(self, index):
        ...
        pseudo = nn.interpolate(pseudo.float(), (img.size[1], img.size[0]), mode="nearest").squeeze(0)
        pseudo = Image.fromarray(pseudo.permute(1, 2, 0).cpu().numpy().astype(np.uint8))
        sam = self.load_sam(path)
        img, pseudo, sam = self.transform(img, pseudo, sam)

        return img, pseudo, sam
    ...
    def load_sam(self, path):
        cate, name = path.split('/')[-2:]
        name = name.replace("JPEG", "npy")
        path = os.path.join(self.sam_path, cate, name)
        sam = np.load(path)
        return sam
