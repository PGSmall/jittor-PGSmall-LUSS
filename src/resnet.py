...
class Mulpixelattn(nn.Module):
    ...
    def execute(self, x):
        ...
        energy = jt.bmm(query, key)
        attention = nn.softmax(energy, dim=2)
        value = self.v(x).view(b, -1, h*w)

        out = jt.bmm(value, attention.permute(0, 2, 1))
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        if train_mode == 'pixelattn':
            ...
            #                             num_classes + 1, 1, 1)
            # eanet + seg_cls
            self.last_layer = EAHead(in_channels=num_out_filters * block.expansion, 
                                     channels=512, num_classes=num_classes + 1)
            # self.sam_layer = conv1x1(256, 512)
    ...
    def execute(self, inputs, gridq=None, gridk=None, mode='train'):
        ...
        #     sam_feats = self.sam_layer(sam_feats)
        #     return self.last_layer(out+sam_feats)
        if self.train_mode == 'finetune':
            out = self.execute_backbone(inputs)
